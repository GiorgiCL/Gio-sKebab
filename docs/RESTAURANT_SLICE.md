...
Other routes, including `/api/admin/**`, remain denied by temporary security. No admin login, editing API, menu, promotion, or image workflow is part of this slice. Owner management will be added with authenticated admin work; initial data must be loaded by an authorized operator through a controlled process until then. No production credentials or business seed data are embedded in migrations.

The current automated integration tests run Flyway and JPA against isolated H2 databases. They cover this contract, schedule precedence, timezone conversion, missing data, security, and key constraints. PostgreSQL-specific migration validation remains required before production release, as stated in the roadmap.
