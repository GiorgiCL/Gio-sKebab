...
Flyway V6 adds category and item translation tables for en/ru/ka. Lithuanian remains in the original required columns. Public menu GET accepts optional `?lang=lt|en|ru|ka`, resolves names and descriptions field by field, and rejects unsupported values with 400. Admin responses include `translations` keyed by locale, with `{name}` for categories and `{name, description}` for items. A supplied map replaces non-Lithuanian translations; an omitted map preserves them for older clients. Price, flags, order, and image URL remain common to all languages.

Fast H2 HTTP tests cover content filtering, ordering, CRUD, validation, image/featured behavior, localization, and security. `mvnw verify` also runs the disposable PostgreSQL 17.9 integration test for V1 through V6 and database constraints. See [BACKEND_FOUNDATION.md](BACKEND_FOUNDATION.md).
