...
Only the three documented public restaurant GET routes and anonymous CSRF/login are publicly permitted. Other `/api/admin/**` requests require authentication; other routes remain denied by default. There are no admin content mutation routes yet. Malformed, blank, or oversized login fields return a generic 400 problem response without echoing credentials. Passwords are limited to 72 UTF-8 bytes so BCrypt never silently truncates them. Spring Security's standard headers remain enabled. No password, hash, or session ID is returned by the API.
