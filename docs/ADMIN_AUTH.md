...
Only the three documented public restaurant GET routes and anonymous CSRF/login are publicly permitted. Other `/api/admin/**` requests require authentication; other routes remain denied by default. Restaurant and opening-hours mutations are documented in [RESTAURANT_SLICE.md](RESTAURANT_SLICE.md). Malformed, blank, or oversized login fields return a generic 400 problem response without echoing credentials. Passwords are limited to 72 UTF-8 bytes so BCrypt never silently truncates them. Spring Security's standard headers remain enabled. No password, hash, or session ID is returned by the API.
