...
The repository started as a minimal Spring Boot application (`BackendApplication`) with one context-load test and a Maven build. The `pom.xml` baseline is Java 21 and Spring Boot 4.1.1, with Spring Web MVC, Security, Data JPA, Validation, Flyway/PostgreSQL support, PostgreSQL runtime driver, DevTools, and Spring Boot test starters. The backend foundation provides local/test/production database configuration, fast isolated H2 tests, and a separate PostgreSQL Testcontainers validation layer; see [BACKEND_FOUNDATION.md](BACKEND_FOUNDATION.md). The first domain slice adds a Flyway migration, restaurant profile, opening hours and overrides, and three public read routes; see [RESTAURANT_SLICE.md](RESTAURANT_SLICE.md). Owner session authentication and first-account provisioning are now present; see [ADMIN_AUTH.md](ADMIN_AUTH.md). Owner content editing, menu, frontend, deployment assets, proxy login limits, recovery, and full CI execution remain future work.
