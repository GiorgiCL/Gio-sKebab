...
The repository started as a minimal Spring Boot application (`BackendApplication`) with one context-load test and a Maven build. The `pom.xml` baseline is Java 21 and Spring Boot 4.1.1, with Spring Web MVC, Security, Data JPA, Validation, Flyway/PostgreSQL support, PostgreSQL runtime driver, DevTools, and Spring Boot test starters. The backend foundation now adds local/test/production database configuration, an isolated H2 smoke test, a migration directory, and temporary deny-all HTTP security; see [BACKEND_FOUNDATION.md](BACKEND_FOUNDATION.md). There are still no domain packages, SQL migrations, API routes, owner authentication, frontend, or deployment assets. Verify exact versions and supported configuration against the repository and official Spring documentation when feature implementation begins.
