...
Run `.\mvnw.cmd verify` (or `./mvnw verify` on Unix) with a working Docker-compatible container runtime for both the fast tests and `*IT` PostgreSQL integration tests. Maven Failsafe runs the latter after Surefire's fast tests. The PostgreSQL test uses Spring Boot's Testcontainers service connection and a disposable `postgres:17.9-alpine` container; JDBC and Flyway both connect to that container. It never uses `gios_kebab_dev`, `DB_PASSWORD`, a local PostgreSQL instance, or any production database. The test checks that V1 through V3 migrate, Hibernate validates the schema, JPA persists the profile and admin account, and PostgreSQL enforces selected schema constraints including menu price and category relationships. Docker is required for `verify`; `test` does not require it. The local PostgreSQL 17.9 startup and Flyway V1 migration were also manually validated before this automated layer was added.

## Security and observability

Owner session authentication now protects `/api/admin/**`; see [ADMIN_AUTH.md](ADMIN_AUTH.md) for provisioning, API, cookie, CSRF, and CORS details. The three restaurant GET endpoints and menu GET remain public. Form login and HTTP Basic are disabled.
